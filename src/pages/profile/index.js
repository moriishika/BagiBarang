  ...
  return (
    <div className="h-full w-full">
      <div className="flex justify-center items-center w-full p-4 bg-white sticky top-0 z-50">
        {session ? (
          <img className="w-20 h-20 rounded-full shadow-lg" src={session.user.image} />
        ) : (
          <svg
        ...
        <div className="w-1/3 ml-5">
          ...
          <div className="flex">
            ...
            <button className="mt-1 ml-1 text-md bg-transparent block border border-red-500 w-1/5 rounded-md hover:text-white text-red-500 font-medium hover:bg-red-500" onClick={signOut}>Keluar</button>
          </div>
        </div>
      </div>
      <Items items={items} />
