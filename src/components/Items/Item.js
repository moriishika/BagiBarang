...
let windowOffset = 0;
const Item = ({inProfile, item, inItemDetail}) => {
  ...
  const createWhatsappLink = (phoneNumber) => {
    const itemName = item.name.trim().replace(/\s+(?=\S)/g, "%20");
    const link = `https://wa.me/${phoneNumber}?text=Saya%20berminat%20dengan%20barang%20yang%20anda%20posting%20di%20*Bagi%20Barang*%20dengan%20nama%20barang%20*${itemName}*%20%20%0A%0Ahttps%3A%2F%2Fbagibarang.com%2Fitems%2Fdetail%2F${item.slug}`
    return link
  }
  ...
  return (
    <div className="w-full xl:w-full md:w-4/5 my-5 p-0 m-auto">
      {isDeleteModalOpened && (
      ...
      <div className="pb-4 flex justify-between items-center">
        <div className="flex items-center">
          <Link href={`/${item.uploader[0].slug}`}>
            <a>
              <img
                src={item.uploader[0].image}
                className="w-12 rounded-full"
              />
            </a>
          </Link>
          <Link href={!session ? "/login" : `/${item.uploader[0].slug}`}>
            <a className="ml-3 font-medium hover:text-blue-600">
              {item.uploader[0].name}
            </a>
          </Link>
        </div>
      ...
      <div>
        ...
        {inItemDetail && (
          <div>
            ...
            <pre className="text-lg my-5">{item.description}</pre>
            ...
            <div className="font-medium">
              ...
              {item?.phoneNumber && (
                <div className="flex items-center mt-3">
                  ...
                  <a className="ml-3 " target="_blank" rel="noopener noreferrer" href={createWhatsappLink(item.phoneNumber)}>{item.phoneNumber}</a>
                </div>
              )}
              {item?.facebook && (
                <div className="flex items-center mt-3">
                  ...
                  <a  target="_blank" rel="noopener noreferrer" href={`https://web.facebook.com/${item.facebook}`} className="ml-3 ">{item.facebook}</a>
                </div>
              )}
              {item?.instagram && (
                <div className="flex items-center mt-3">
                  ...
                  <a target="_blank" rel="noopener noreferrer" href={`https://www.instagram.com/${item.instagram}`} className="ml-3">{item.instagram}</a>
                </div>
              )}
              {item?.twitter && (
                <div className="flex items-center mt-3">
                  ...
                  <a  target="_blank" rel="noopener noreferrer" href={`https://twitter.com/${item.twitter}`} className="ml-3">{item.twitter}</a>
                </div>
              )}
              {item?.email && (
              <div className="flex items-center mt-3">
                ...
                <a className="ml-3" target="_blank" rel="noopener noreferrer" href={`mailto:${item.email}?subject=Bagi Barang - Saya Peminat Barang Anda &body=Saya berminat dengan barang yang anda posting di Bagi Barang dengan nama ${item.name}`}>{item.email}</a>
              </div>
              )}
              <div className="flex items-center mt-3">
                <svg
                ...
                <p className="ml-3">{item.address}</p>
              </div>
              <div className="w-full mt-8 flex justify-center">
                <ShareButton
                  title={item.name}
                  url={`https://bagibarang.com/items/detail/${item.slug}`}
                ></ShareButton>
              </div>
            </div>
          </div>
      ...
      {!inItemDetail && (
        <div className="flex w-full justify-between text-white text-3xl mt-4">
          {(!inProfile || item.user_id !== session?.user?.id) && (
            <>
              <ShareButton
                title={item.name}
                url={`https://bagibarang.com/items/detail/${item.slug}`}
              ></ShareButton>
              <Link
                href={!session ? "/login" : `/items/detail/${item.slug}`}
              >
                <a className="bg-green-500 w-4/5 p-2 rounded-md hover:bg-green-700 duration-150 text-center">
                  Detail
                </a>
              </Link>
              {/* <div onClick={() => router.push(!session ? '/login' : `/items/detail/${item.slug}`)} className="bg-green-500 w-4/5 p-2 rounded-md hover:bg-green-700 duration-150 text-center">
                  Detail
                </div> */}
            </>
          )}
          ...
          {inProfile && item.user_id === session?.user?.id && (
            <>
              ...
              <Link href={`/items/update/${item.slug}`}>
              ...
              <Link href={`/items/detail/${item.slug}`}>
